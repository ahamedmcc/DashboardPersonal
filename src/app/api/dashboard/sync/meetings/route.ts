...
import {
  hermesMeetingsPayloadSchema,
  syncHermesMeetings,
  type HermesMeeting,
} from "@/lib/sync/hermes-meetings";

/**
 * POST /api/dashboard/sync/meetings
 *
 * Receives the raw contents of Hermes's meetings.json (running on Hostinger)
 * and reconciles it into the Meeting table. See src/lib/sync/hermes-meetings.ts
...
export async function POST(request: NextRequest) {
  ...
  const parsed = hermesMeetingsPayloadSchema.safeParse(body);
  if (!parsed.success) return fromZodError(parsed.error);

  const items: HermesMeeting[] = Array.isArray(parsed.data)
    ? parsed.data
    : "items" in parsed.data
      ? parsed.data.items
      : "meetings" in parsed.data
        ? parsed.data.meetings
        : [];

  // Always log a TelegramEvent so the inbox shows the sync attempt.
  const event = await prisma.telegramEvent.create({
    data: {
      rawMessage: body as Prisma.InputJsonValue,
      classifiedType: "hermes-sync",
      processedStatus: "pending",
    },
  });

  try {
    const summary = await syncHermesMeetings(items);
